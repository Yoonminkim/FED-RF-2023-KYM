...
function sortingFn() {
  ...
  // 2. 정렬기준값 읽기 : idx, tit, cont (객체 데이터 배열의 속성명)
...
// - 이벤트대상 : .sbtn
const sbtn = dFn.qs('.sbtn');
const sample = dFn.qs('.sample');
...
function searchingFn(){
  ...
  console.log(stxt, cta);

  // 3. 검색 결과가 있으면 

  let res = list2.filter(v=>{
    // v[객체속성명] -> cta 변수에 idx/tit/cont 중 하나
    // -> cta 변수에 idx/tit/cont 중 하나들어옴
    // indexOf(검색어) -> indexOf (stxt 변수값)
...
dFn.addEvt(sample, 'click', ()=>{
  ...
  let res1 = list2.find(v=>{
    ...
    // 데이터가 완벽하게 일치하지 않으면 undifined 리턴! 
  });
  console.log('검색어 결과:', res1);
  
  // 2. find() 메서드 LIKE 검색하기 : 데이터의 일부로 찾음!
